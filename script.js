...
window.onload = async function() {
    try {
        ...
        const headerAuthBtn = document.querySelector('#auth-section button');
        if (headerAuthBtn) {
    ...
    modalData.innerHTML = `
        <div class="modal-layout" style="display:flex;">
            ...
            <div class="modal-info-side" style="flex:1.2; padding:30px; color:black;">
                ...
                <ul class="modal-specs" style="list-style:none; padding:0; color:#444; margin: 15px 0;">
                    <li><b>Розробник:</b> ${d.author}</li>
                    <li><b>Рік:</b> ${d.year}</li>
                    <li><b>Вимоги:</b> ${d.specs}</li>
                </ul>
                <button class="modal-buy-btn" style="width:100%; padding:15px; background:#4a3427; color:white; border:none; cursor:pointer; font-weight:bold; border-radius:8px;" onclick="addToCartDirect('${d.title}', ${d.price}, '${d.img}')">ДОДАТИ В КОШИК</button>
            </div>
        </div>
    `;
    openMainModal();
